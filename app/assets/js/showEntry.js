$.get("/todos",function(todos) {
	$.each(todos,function(index, product) {
		...
		$("tr:last").last().append("<td>" + "<button  type=\"button\" class=\"btn btn-default\" onclick=\" $.ajax({url:'/changeTodo/" + product.id + "', type: 'PUT', success: location.reload()});\"> <span class=\"glyphicon glyphicon-pencil\" aria-hidden=\"true\"></span> </button> " +
